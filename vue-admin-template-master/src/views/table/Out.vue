<template>
  <div class="app-container">
    <el-card shadow="hover">
      <el-row>
        <el-col :span="17" style="margin-bottom: 10px"><el-tag effect="plain" style="margin-bottom: 5px">出库信息管理</el-tag></el-col>
      ...
      <el-table :data="tableData" border stripe max-height="500" style="width: 100%"  highlight-current-row @current-change="handleCurrentChange" ref="singleTable">
        ...
        <el-table-column prop="man" label="经手人"></el-table-column>
    ...
    <el-dialog title="新增出库材料信息" :visible.sync="dialogFormVisible">
...
<script>
  ...
  import {
    addInWare, addOutWare,
    deleteInWare, deleteOutWare,
    findInWare,
    findInWareByUid,
    findOutWare,
    findOutWareByUid,
    updateInWare, updateOutWare
  } from "../../api/table";
  ...
  export default {
    ...
    created() {
      if(this.role){
        ...
        findOutWareByUid(data).then(res=>{
          this.tableData = res.data.list
        })
      }else {
        findOutWare().then(res=>{
    ...
    methods:{
      ...
      handleDelete(index,item){
        ...
        data.append("outTime",item.outTime)
        data.append("oid",item.oid)
        data.append("uid",item.uid)
        data.append("id",item.id)
        deleteOutWare(data).then(res=>{
          if(res.data.success){
